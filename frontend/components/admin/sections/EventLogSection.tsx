...
export default function EventLogSection() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [s, d] = await Promise.all([
        api.get<Stats>("/api/admin/event-log/stats"),
        api.get<PageData>(`/api/admin/event-log?${params}`),
      ]);
      setStats(s);
      setItems(d.items);
      setTotal(d.total);
    ...
    setLoading(false);
  }, [page, eventType, dateFrom, dateTo, search]);

  useEffect(() => { fetchAll(); }, [fetchAll]);
  useEffect(() => { setPage(1); }, [eventType, dateFrom, dateTo, search]);

  const columns: Column<EventLogItem>[] = useMemo(() => [
    { key: "eventType", header: "Tipo", width: "90px", cell: (e) => <span className={`type-badge ${TYPE_CLS[e.eventType] ?? ""}`} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 999, fontFamily: "var(--font-mono)", textTransform: "uppercase", fontWeight: 600 }}>{e.eventType}</span> },
    { key: "action", header: "Azione", grow: true, cell: (e) => <span style={{ fontSize: 13 }}>{e.action}</span> },
    { key: "entity", header: "Entità", width: "130px", mono: true, cell: (e) => e.entity ? <span style={{ fontSize: 12 }}>{e.entity}/{e.entityId}</span> : "—" },
  ...
  const actions: RowAction<EventLogItem>[] = useMemo(() => [
    ...
    },
    {
      ...
      tooltip: () => "Timeline",
      onClick: (e) => { if (e.entity && e.entityId) setTimelineEntity({ entity: e.entity, entityId: e.entityId }); },
      hidden: (e) => !e.entity || !e.entityId,
    },
  ], []);
  ...
  return (
    <>
      <AdminTopBar title="Log eventi" searchValue={search} onSearchChange={setSearch} searchPlaceholder="Cerca azione, entità..."
        filter={eventType} onFilterChange={setEventType} filterOptions={EVENT_TYPES}
        leading={
