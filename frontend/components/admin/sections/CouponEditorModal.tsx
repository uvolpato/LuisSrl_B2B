...
export default function CouponEditorModal({ onClose, onSaved, initial }: { onClose: () => void; onSaved: () => void; initial?: any }) {
  ...
  const [raccolte, setRaccolte] = useState<string[]>([]);

  useEffect(() => {
    api.get<any[]>("/api/admin/famiglie").then(r => setFamiglie((r || []).map((f: any) => f.nome || f.codice))).catch(() => {});
    api.get<any[]>("/api/admin/raccolte").then(r => setRaccolte((r || []).map((r2: any) => r2.nome || r2.slug))).catch(() => {});
  }, []);
  ...
  return (
    <Modal open size="sm" onClose={onClose} noHeader>
        ...
        <>
          <div className="modal-root-body" style={{ padding: "24px 28px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div><label style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500, display: "block", marginBottom: 4 }}>Dettaglio ambito</label><select style={{ ...stl, opacity: scope === "all" || type === "free-ship" ? 0.4 : 1, pointerEvents: scope === "all" || type === "free-ship" ? "none" : "auto" }} value={scopeDetail} onChange={e => setScopeDetail(e.target.value)} disabled={type === "free-ship"}><option value="">Seleziona...</option>{scope === "family" && famiglie.map(f => <option key={f} value={f}>{f}</option>)}{scope === "collection" && raccolte.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
      ...
      {step === "destinatari" && (
        <>
          <div className="modal-root-body" style={{ padding: "24px 28px", display: "flex", flexDirection: "column", overflow: "auto", flex: 1 }}>
            ...
            {isEdit ? (
              ...
              <div style={{ position: "relative", flexShrink: 0, marginBottom: 6 }}>
                <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "var(--muted)", zIndex: 1, lineHeight: 0 }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></span>
                <input type="text" value={targetSearch} onChange={e => setTargetSearch(e.target.value)} placeholder="Cerca per codice o ragione sociale..." style={{ width: "100%", padding: "8px 36px 8px 32px", border: "1px solid var(--border)", borderRadius: 8, background: "var(--surface)", font: "inherit", fontSize: 13, color: "var(--fg)", outline: "none", boxSizing: "border-box" }} />
                {targetSearch && <span onClick={() => setTargetSearch("")} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", cursor: "pointer", color: "var(--muted)", lineHeight: 0, fontSize: 16, zIndex: 1 }}>×</span>}
              </div>
              <DataTable
                columns={[
