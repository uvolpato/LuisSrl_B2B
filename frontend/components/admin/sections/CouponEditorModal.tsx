        ...
        <>
          <div className="modal-root-body" style={{ padding: "24px 28px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div><label style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500, display: "block", marginBottom: 4 }}>Valore</label><input style={stl} type="number" min={1} value={value} onChange={e => setValue(Number(e.target.value) || 0)} disabled={type === "free-ship"} /></div>
              <div><label style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500, display: "block", marginBottom: 4 }}>Ambito</label><select style={stl} value={scope} onChange={e => setScope(e.target.value)}><option value="all">Tutto il catalogo</option><option value="family">Famiglia specifica</option><option value="collection">Raccolta</option></select></div>
              <div><label style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500, display: "block", marginBottom: 4 }}>Dettaglio ambito</label><select style={{ ...stl, opacity: scope === "all" ? 0.4 : 1, pointerEvents: scope === "all" ? "none" : "auto" }} value={scopeDetail} onChange={e => setScopeDetail(e.target.value)}><option value="">Seleziona...</option>{scope === "family" && ["Vasi in terracotta","Cotto portoghese","Fioriere"].map(f => <option key={f} value={f}>{f}</option>)}{scope === "collection" && ["Novità 2026","Natale","Primavera"].map(f => <option key={f} value={f}>{f}</option>)}</select></div>
              <div><label style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500, display: "block", marginBottom: 4 }}>Soglia min. ordine</label><input style={stl} type="number" min={0} value={minOrder} onChange={e => setMinOrder(e.target.value)} placeholder="Nessuna soglia" /></div>
      ...
      {step === "destinatari" && (
        <>
          <div className="modal-root-body" style={{ padding: "24px 28px" }}>
            ...
            {searchResults.length > 0 && (<div style={{ maxHeight: 200, overflow: "auto", border: "1px solid var(--border)", borderRadius: 8, marginBottom: 12 }}>{searchResults.map((c: any) => (<div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", borderBottom: "1px solid var(--border)", fontSize: 13, cursor: "pointer" }} onClick={() => toggleClient(c.id)}><input type="checkbox" checked={selectedIds.has(c.id)} readOnly style={{ accentColor: "var(--accent)" }} /><span style={{ flex: 1 }}>{c.ragioneSociale || c.nome}</span><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--muted)" }}>{c.cod}</span></div>))}</div>)}
