...
export default function CouponEditorModal({ onClose, onSaved, initial }: { onClose: () => void; onSaved: () => void; initial?: any }) {
  ...
  useEffect(() => {
    api.get<any[]>("/api/admin/famiglie").then(r => setFamiglie(r || [])).catch(() => {});
    api.get<any[]>("/api/admin/raccolte").then(r => setRaccolte(r || [])).catch(() => {});
  }, []);
        ...
        <>
          <div className="modal-root-body" style={{ padding: "24px 28px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div><label style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500, display: "block", marginBottom: 4 }}>Dettaglio ambito</label><select style={{ ...stl, opacity: scope === "all" || type === "free-ship" ? 0.4 : 1, pointerEvents: scope === "all" || type === "free-ship" ? "none" : "auto" }} value={scopeDetail} onChange={e => setScopeDetail(e.target.value)} disabled={type === "free-ship"}><option value="">Seleziona...</option>{scope === "family" && famiglie.map((f: any) => <option key={f.codice} value={f.codice}>{f.nome || f.codice}</option>)}{scope === "collection" && raccolte.map((r: any) => <option key={r.slug || r.codice} value={r.codice || r.slug}>{r.nome || r.slug}</option>)}</select></div>
