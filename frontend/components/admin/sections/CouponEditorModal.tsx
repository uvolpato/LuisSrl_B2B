      ...
      {step === "destinatari" && (
        <>
          <div className="modal-root-body" style={{ padding: "24px 28px" }}>
            ...
            {searchResults.length > 0 && (<div style={{ maxHeight: 200, overflow: "auto", border: "1px solid var(--border)", borderRadius: 8, marginBottom: 12 }}>{searchResults.map((c: any) => (<div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", borderBottom: "1px solid var(--border)", fontSize: 13, cursor: "pointer" }} onClick={() => toggleClient(c.id)}><input type="checkbox" checked={selectedIds.has(c.id)} readOnly style={{ accentColor: "var(--accent)", flexShrink: 0 }} /><span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.ragioneSociale || c.nome}</span><span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--muted)", flexShrink: 0 }}>{c.cod || c.codiceCliente}</span></div>))}</div>)}
