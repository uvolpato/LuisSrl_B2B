...
export default function RaccolteSection() {
  ...
  const [allArticles, setAllArticles] = useState<ArticoloSlim[]>([]);
  ...
  async function openCreateModal() {
    try {
      const arts = await api.get<ArticoloSlim[]>("/api/integrazione/articoli");
      setAllArticles(arts);
    } catch {
      setAllArticles([]);
    }
  ...
  async function openEditModal(id: number) {
    try {
      const [detail, arts] = await Promise.all([
        ...
        api.get<ArticoloSlim[]>("/api/integrazione/articoli"),
      ]);
  ...
  const columns: Column<Raccolta>[] = [
    ...
    },
    {
      ...
      width: "200px",
      cell: (r) => (
        <span style={{ color: "var(--muted)", fontSize: 13, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>
          {r.descrizione || "—"}
        </span>
      ),
    },
...
function RaccoltaEditModal({
  ...
  const filteredArticles = allArticles.filter((a) => {
    if (articleFilter === "attivi" && a.stato !== "ATTIVO" && a.stato !== "attivo") return false;
    if (articleFilter === "nascosti" && a.stato !== "NASCOSTO" && a.stato !== "nascosto") return false;
    ...
    return a.nome.toLowerCase().includes(q) || a.codiceLinea.toLowerCase().includes(q);
  });
  ...
  return (
    <>
          ...
          <div style={{ overflow: "auto", paddingRight: 24, borderRight: "1px solid var(--border)" }}>
            ...
            <div className="field" style={{ marginBottom: 10 }}>
              ...
              <div
                ...
                onDrop={(e) => { e.preventDefault(); e.currentTarget.style.borderColor = "var(--border)"; const f = e.dataTransfer.files[0]; if (f) handleImageFile(f); }}
                onClick={() => document.getElementById("raccolta-image-input")?.click()}
              >
                <input
                ...
                />
                {previewUrl || immagineUrl ? (
                  <div style={{ position: "relative" }}>
                    <img
                      src={previewUrl || immagineUrl}
                      alt="Anteprima"
                      style={{ width: "100%", maxHeight: 100, objectFit: "contain", borderRadius: "var(--radius)", display: "block", cursor: "pointer" }}
                      onClick={(e) => { e.stopPropagation(); setZoomedImage(previewUrl || immagineUrl); }}
                    />
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      className="btn btn-ghost btn-sm"
                      style={{ marginTop: 4 }}
                      onClick={(e) => { e.stopPropagation(); setPendingImage(null); setPreviewUrl(null); setImmagineUrl(""); }}
                    >
                      Rimuovi immagine
                    </button>
                  </div>
              ...
              <DataTable
                columns={[
                    ...
                    cell: (a: ArticoloSlim) => (
                      <div className="cell-entity">
                        ...
                        <div className="cell-entity-text">
                          ...
                          <span className="cell-entity-title">{a.name ?? a.nome}</span>
                  ...
                  },
                  {
                    ...
                    align: "center",
                    cell: (a: ArticoloSlim) => (
                      <span className={`status ${a.stato === "ATTIVO" || a.stato === "attivo" ? "status-active" : "status-hidden"}`}>
                        {a.stato === "ATTIVO" || a.stato === "attivo" ? "attivo" : "nascosto"}
                      </span>
                    ),
                  },
                  {
                    ...
                    align: "center",
                    mono: true,
                    cell: (a: ArticoloSlim) => a.variantiCount ?? 0,
                  },
                  {
                    key: "raccolte",
                    header: "Raccolte",
                    ...
                    align: "center",
                    mono: true,
                    cell: (a: ArticoloSlim) => a.raccolte?.length ?? 0,
                  },
                ]}
                rows={filteredArticles}
                rowKey={(a) => a.articoloId}
      ...
      {zoomedImage && (
        ...
        >
          <img src={zoomedImage} alt="Ingrandimento" style={{ maxWidth: "90vw", maxHeight: "90vh", objectFit: "contain", borderRadius: 8 }} />
        </div>
      )}
    </>);
}
