...
- Frontend: modale "Ricerca intelligente" collegata; risultati nella griglia catalogo con spinner+blocco durante la ricerca.
- **Ricerca per immagine** (`POST /api/catalogo/ricerca-immagine`): la foto del cliente NON viene embeddata con CLIP; Gemini Vision estrae gli attributi oggettivi con guardrail stretti (solo il contenitore, vocabolari chiusi materiale/forma/uso, niente invenzioni, niente misure in cm, `pertinente:false` se non è un prodotto) → `{attributi, keywords}` → **stessa** pipeline di ranking del testo (embed keywords + boost attributi). Attivo nel catalogo; in dashboard resta "in arrivo" (un file non si passa via navigazione). Verificato in dev: foto vaso marrone → ARGO NOCCIOLA in cima.
- **Query rewrite + boost attributi oggettivi** (`SEARCH_QUERY_REWRITE=on|off`): prima di embeddare, Gemini riscrive la query in keyword normalizzate di dominio ed **estrae gli attributi oggettivi** (colore, materiale, forma, dimensione, uso, tipo — es. "marrone chiaro"→"nocciola"), scartando i termini soggettivi ("elegante"). Gli attributi vanno in testa e ripetuti nel testo embeddato; poi ogni attributo presente nei **dati oggettivi** dell'articolo (nome + colore + famiglia/linea via `objtext`) dà un bonus di ranking (`SEARCH_ATTR_BOOST` per attributo, default 0.10; tetto `SEARCH_BOOST_CAP` 0.30). Così più attributi richiesti soddisfa l'articolo, più sale. Verificato in dev: "contenitore blu in cotto" → ARGO BLU 0.99 (blu+cotto), "elegante per il salotto" → nessun attributo forzato. Costo: +1 chiamata Gemini per ricerca; fallback alla query grezza se non disponibile.
