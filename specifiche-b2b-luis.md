...
## 15. Indicazioni tecnologiche e piano di hosting

### 15.1 Stack tecnologico proposto

| Layer | Tecnologia | Motivazione |
|-------|-----------|-------------|
| **Frontend / BFF** | Next.js (App Router) | Full-stack, SSR, API routes unificate |
| **Backend AI** | Python (FastAPI) | Integrazione nativa con librerie ML/AI |
| **Database** | PostgreSQL + pgvector | Embedding vettoriali per ricerca semantica direttamente nel DB |
| **Cache** | Redis (Upstash) | Cache embedding, sessioni, rate limiting |
| **File storage** | Cloudflare R2 / S3 | Immagini (sfondo bianco + ambientate) |
| **Vector DB** | pgvector (su PostgreSQL) | Volume stimato < 1M articoli, non serve DB vettoriale separato |

### 15.2 Modelli LLM per inferenza locale

Strategia: **inferenza self-hostata su GPU a consumo** per i carichi di lavoro AI del portale. La generazione di immagini ambientate (§11.5.2) resta affidata a **API esterne** (DALL·E, Stable Diffusion, Midjourney).

#### 15.2.1 Modelli consigliati

| Modello | Parametri | VRAM (Q4) | Tokens/s su RTX 4090 | Casi d'uso | Licenza |
|---------|-----------|-----------|---------------------|------------|---------|
| **Qwen 3.5 27B** | 27B dense | ~16 GB | 25-35 tok/s | Creatore descrizione, banner, suggerimenti | Apache 2.0 |
| **Qwen 3.6 35B-A3B** | 35B MoE (3B attivi) | ~22 GB | 80-100 tok/s | Ricerca testuale veloce | Apache 2.0 |
| **Mistral Small 3.1** | 24B | ~14 GB | 30-40 tok/s | Alternativa general-purpose | Apache 2.0 |

Tutti i modelli supportano il **multilingua** (italiano incluso) e hanno licenza Apache 2.0 (nessuna restrizione commerciale).

Modelli con **visione** (per ricerca per immagini §12.1.2): Qwen 3.6 27B/35B ha encoder vision nativo, richiede +1-2 GB VRAM extra.

Per **embedding vettoriali** (ricerca semantica §12.1.1): modello `bge-m3` (self-hosted su CPU, ~2 GB RAM) o API OpenAI (`text-embedding-3-small`, ~$0.02/1K input).

#### 15.2.2 Fornitore GPU Cloud a consumo

| Fornitore | GPU | On-demand/hr | Spot/hr | Minimo fatturazione | Affidabilità |
|-----------|-----|-------------|---------|-------------------|-------------|
| **RunPod** | RTX 4090 24GB | $0.69 | $0.34 | 1 secondo | Alta (Secure Cloud SLA) |
| **RunPod** | A100 80GB | $1.89 | $1.19 | 1 secondo | Alta |
| **RunPod** | L40S 48GB | $0.86 | n/d | 1 secondo | Alta |
| **Vast.ai** | RTX 4090 24GB | $0.27-0.36 | n/d | 1 minuto | Media (P2P marketplace) |
| **Lambda Labs** | A100 80GB | $2.49 | n/d | 1 minuto | Molto alta |

**Scelta consigliata: RunPod** (Serverless tier) per:
- fatturazione al secondo e scala a zero (si paga solo durante l'inferenza);
- due livelli: Secure Cloud (SLA, datacenter) e Community Cloud (più economico, ~40% in meno);
- storage persistente a $0.07/GB/mese;
- FlashBoot: cold start < 250 ms per container piccoli.

### 15.3 Stima costi mensili

#### 15.3.1 Infrastruttura fissa (sito)

| Voce | Fornitore | Costo/mese |
|------|-----------|-----------|
| Hosting Next.js | Vercel Pro o Railway | $0-20 |
| Database PostgreSQL | Supabase (free tier 500MB) o Pro (8GB) | $0-25 |
| File storage immagini | Cloudflare R2 (10GB free) | $0-5 |
| Cache Redis | Upstash (free tier 10MB) | $0 |
| Dominio + DNS | — | ~$2 |
| **Subtotale hosting** | | **~$2-52/mese** |

#### 15.3.2 GPU AI (RunPod Serverless)

| Scenario | Utilizzo | Costo GPU/mese |
|----------|----------|---------------|
| **Solo admin** (creatore descrizione + aggiornamento indice) | ~5-10 ore effettive/mese | $10-20 |
| **Admin + clienti** (ricerca semantica + per immagini, ~2h/giorno) | ~60 ore/mese | $60-120 |
| **Completo** (admin + clienti + banner + raccomandazioni AI) | — | $75-165 |

#### 15.3.3 API esterne

| Servizio | Costo |
|----------|-------|
| Generazione immagini ambientate (DALL·E 3 / SD API, ~$0.04/immagine) | $10-60/mese (variabile) |
| Embedding testuali (se non self-hosted) | ~$2/mese |

#### 15.3.4 Totali mensili stimati

| Scenario | Hosting | GPU AI | API immagini | **Totale** |
|----------|---------|--------|-------------|-----------|
| Solo admin | $2-52 | $10-20 | $0 | **~$12-72/mese** |
| Admin + clienti | $25-52 | $60-120 | $10-30 | **~$95-202/mese** |
| Completo | $25-52 | $75-165 | $20-60 | **~$120-277/mese** |

#### 15.3.5 Confronto con API esterne (Claude / GPT-4o)

| Soluzione | Costo medio/mese |
|-----------|-----------------|
| Claude API (solo descrizione + banner) | $150-300 |
| GPT-4o (completo con vision) | $200-400 |
| **Self-hosted su RunPod (Qwen 27B)** | **$75-165** |

> Il break-even con le API esterne si raggiunge in 2-4 mesi per carichi sostenuti.
